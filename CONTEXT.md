...
Solid Doctor is a diagnostic product for Solid and SolidStart code health. It is not a formatter, a general lint suite, or an ESLint-first port. It should feel native to Solid and natural beside `oxlint` and `oxfmt`.

## Product Promise

Fast diagnostics for Solid reactivity, SSR and lifecycle bugs.

The tool should make Solid's correctness and maintainability model visible across local development, CI, code review, and agentic coding workflows.

## Core Users

- Solid developers who want fast feedback on components, effects, resources, stores, and SolidStart routes.
- Teams adopting or maintaining Solid who need guardrails against patterns that compile but become stale, unsafe, leaky, or hard to maintain.
...
- AI coding-agent users who need generated Solid code to follow Solid's mental model instead of generic frontend habits.
...
- Node-compatible npm package artifact for `npx solid-doctor` and `bunx solid-doctor`.
- Type checks use `tsgo` through `@typescript/native-preview`.
- The CLI entry point is `src/cli.ts`.
- The packaged CLI artifact is `dist/cli.js`, generated by `bun run build`.
